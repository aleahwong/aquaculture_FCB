...
library(dplyr)
library(rfishbase)
sppdata<-read.csv("/Users/aleahw/Documents/Master's!!/FCB_traits.csv")
...
foodtraits <- sppdata[, c("species", "agg_trophic", "spatial", "temp_range_c", "pO2_min_mbar", "N_range_µmol.kg", "P_range_µmol.kg", "growth_rate_K", "age_mat_years", "max_size_cm", "feed_eff_trophic", "abs_fecundity", "micron", "macron")]
...
#fuzzify exposure variables  
  ...
  # Trait_4_levels[is.na(Trait_4_levels)] <- 0 #turn NAs into 0
}
```
...
f_quantiles <- apply(footraitscopy[2:14] , 2 , quantile, probs = quants , na.rm = TRUE )
...
food_long2$level_val[food_long2$level_val <0.2] <- 0
...
# # NAME<- sprintf("/Users/aleahw/Documents/Master's!!/Fuzzy/foodsecurity_scores_V2.csv")
...
climatelong2$level_val[climatelong2$level_val <0.2] <- 0
...
# NAME<- sprintf("/Users/aleahw/Documents/Master's!!/Fuzzy/climate_scores_V2.csv")
# write.csv(scores2,file=NAME)
...
for (i in 1:nrow(biodiv_long)) {
  ifelse(biodiv_long$value[i] == 0, biodiv_long$low[i] <- 0.5, biodiv_long$low)
  ifelse(biodiv_long$value[i] == 0, biodiv_long$vhigh[i] <- 0, biodiv_long$vhigh)
...
#add the <0.2 membership clause here
biodivlong2$level_val[biodivlong2$level_val <0.2] <- 0
...
# NAME<- sprintf("/Users/aleahw/Documents/Master's!!/Fuzzy/biodiv_scores_V2.csv")
# write.csv(scores2,file=NAME)
```

```{r}
#combine the three numbers into one dataset with some of the other columns in sppdata
Food <- read.csv("/Users/aleahw/Documents/Master's!!/Fuzzy/foodsecurity_scores_V2.csv")
Climate <- read.csv("/Users/aleahw/Documents/Master's!!/Fuzzy/climate_scores_V2.csv")
Biodiversity <- read.csv("/Users/aleahw/Documents/Master's!!/Fuzzy/biodiv_scores_V2.csv")
...
FCB
#save this: 
NAME<- sprintf("/Users/aleahw/Documents/Master's!!/Fuzzy/FCB_scores_V2.csv")
...
orig <- read.csv("/Users/aleahw/Documents/Master's!!/Fuzzy/foodsecurity_scores_V2.csv")
orig <- orig[, c("species", "final")]

# colnames(foodtraits)
#don't forget to reset the food traits code chunk (with pivot)

listt <- c("agg_trophic", "spatial", "temp_range_c", "pO2_min_mbar", "N_range_µmol.kg", "P_range_µmol.kg", "growth_rate_K", "age_mat_years", "max_size_cm", "feed_eff_trophic", "abs_fecundity", "micron", "macron")
...
#save this: 
# NAME<- sprintf("/Users/aleahw/Documents/Master's!!/Fuzzy/F_jacknife_trials_ntrait/jackknife_f_number_trial50.csv")
...
setwd("/Users/aleahw/Documents/Master's!!/Fuzzy/F_jacknife_trials_ntrait")
F_trials <- list.files("/Users/aleahw/Documents/Master's!!/Fuzzy/F_jacknife_trials_ntrait")
F_trials

for (i in 1) { #to initialize the new dataframe
  name <- F_trials[i]
  
  trial <- read.csv(paste("/Users/aleahw/Documents/Master's!!/Fuzzy/F_jacknife_trials_ntrait", name, sep="/"), header = FALSE)
  
  colnames(trial)<-trial[1,]
  trial<-trial[-1,]
  colnames(trial)[3:15]<-c("zero","one","two", "three", "four","five","six","seven","eight","nine","ten","eleven","twelve")
  
  new_frame <-trial[, 2:15]
  
  library(dplyr)
  new_frame <- new_frame %>% mutate_at(c("zero","one","two", "three", "four","five","six","seven","eight","nine","ten","eleven","twelve"), as.numeric)
}
...
for (i in c(2:(length(F_trials)))) {
  ...
  trial_new <-trial[, 2:15]
  trial_new <- trial_new %>% mutate_at(c("zero","one","two", "three", "four","five","six","seven","eight","nine","ten","eleven","twelve"), as.numeric)
...
#make long
new_frame <- new_frame[, c(1, 3:14)]

new_frame <- new_frame %>% pivot_longer(!c('species'), names_to = 'n_traits_removed', values_to = 'score') %>% ungroup()

plot <- ggplot(new_frame, aes(x = factor(n_traits_removed, level=c("one","two", "three", "four","five","six","seven","eight","nine","ten","eleven","twelve")), y = score)) + 
...
# ggsave("F_ntraits.png", plot, height = 5, width = 8, dpi = 700)
```
...
#initialize dataframe with species and original score
orig <- read.csv("/Users/aleahw/Documents/Master's!!/Fuzzy/climate_scores_V2.csv")
orig <- orig[, c("species", "final")]
...
#save this: 
# NAME<- sprintf("/Users/aleahw/Documents/Master's!!/Fuzzy/C_jacknife_trials_ntrait/jackknife_c_number_trial1.csv")
# write.csv(orig,file=NAME)
```
...
setwd("/Users/aleahw/Documents/Master's!!/Fuzzy/C_jacknife_trials_ntrait")
C_trials <- list.files("/Users/aleahw/Documents/Master's!!/Fuzzy/C_jacknife_trials_ntrait")
C_trials

for (i in 1) { #to initialize the new dataframe
  name <- C_trials[i]
  
  trial <- read.csv(paste("/Users/aleahw/Documents/Master's!!/Fuzzy/C_jacknife_trials_ntrait", name, sep="/"), header = FALSE)
...
#SAVE
# setwd("/Users/aleahw/Documents/Master's!!/Fuzzy/Figures")
# ggsave("C_ntraits.png", plot, height = 5, width = 8, dpi = 700)
```
...
#initialize dataframe with species and original score
orig <- read.csv("/Users/aleahw/Documents/Master's!!/Fuzzy/biodiv_scores_V2.csv")
orig <- orig[, c("species", "final")]
...
#save this: 
# NAME<- sprintf("/Users/aleahw/Documents/Master's!!/Fuzzy/B_jacknife_trials_ntrait/jackknife_b_number_trial1.csv")
# write.csv(orig,file=NAME)
```
...
setwd("/Users/aleahw/Documents/Master's!!/Fuzzy/B_jacknife_trials_ntrait")
B_trials <- list.files("/Users/aleahw/Documents/Master's!!/Fuzzy/B_jacknife_trials_ntrait")
B_trials

for (i in 1) { #to initialize the new dataframe
  name <- B_trials[i]
  
  trial <- read.csv(paste("/Users/aleahw/Documents/Master's!!/Fuzzy/B_jacknife_trials_ntrait", name, sep="/"), header = FALSE)
...
#SAVE
# setwd("/Users/aleahw/Documents/Master's!!/Fuzzy/Figures")
# ggsave("B_ntraits.png", plot, height = 5, width = 8, dpi = 700)
```
...
colnames(orig)[11] = "f-max_size_cm"
colnames(orig)[12] = "f-feed_eff_trophic"
colnames(orig)[13] = "f-abs_fecundity"
colnames(orig)[14] = "f-micron"
colnames(orig)[15] = "f-macron"

#save this: 
NAME<- sprintf("/Users/aleahw/Documents/Master's!!/Fuzzy/jackknife_f_trait.csv")
write.csv(orig,file=NAME)
```
...
dat <- read.csv("/Users/aleahw/Documents/Master's!!/Fuzzy/jackknife_f_trait.csv")
...
plot <- ggplot(dat_long, aes(x = trait_removed, y = score)) +
    ...
    scale_x_discrete(labels = c("Trophic Level (aggression)", "Spatial Behavior", "Temp Range", "pO2 Range", "N Range", "P Range", "Growth Rate", "Age at Maturity", "Max Length", "Trophic Level (feed efficiency)", "Absolute Fecundity", "Micronutrients", "Macronutrients"))+
    theme_classic() + 
...
# # setwd("/Users/aleahw/Documents/Master's!!/Fuzzy/Figures")
# ggsave("F_traitsremoved.png", plot, height = 5, width = 8, dpi = 700)
```
...
#save this: 
NAME<- sprintf("/Users/aleahw/Documents/Master's!!/Fuzzy/jackknife_c_trait.csv")
write.csv(orig,file=NAME)
```
...
dat <- read.csv("/Users/aleahw/Documents/Master's!!/Fuzzy/jackknife_c_trait.csv")
...
#SAVE
setwd("/Users/aleahw/Documents/Master's!!/Fuzzy/Figures")
ggsave("C_traitsremoved.png", plot, height = 5, width = 8, dpi = 700)
```
...
#save this: 
NAME<- sprintf("/Users/aleahw/Documents/Master's!!/Fuzzy/jackknife_b_trait.csv")
write.csv(orig,file=NAME)
```
...
dat <- read.csv("/Users/aleahw/Documents/Master's!!/Fuzzy/jackknife_b_trait.csv")
...
plot <- ggplot(dat_long, aes(x = trait_removed, y = score)) +
    ...
    scale_x_discrete(labels = c("Latitudinal Range", "Water Area", "Absolute Fecundity", "Growth Rate", "Trophic Level (aggression)", "Trophic Level (Feed Efficiency)", "Bioremediation Potential", "Structural Provisioning Potential"))+
    theme_classic() + 
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), 
          axis.title.x = element_text(vjust=20, hjust = 1.4),
          text = element_text(size = 15), axis.text = element_text(size = 8), 
...
#SAVE
setwd("/Users/aleahw/Documents/Master's!!/Fuzzy/Figures")
ggsave("B_traitsremoved.png", plot, height = 5, width = 8, dpi = 700)
```
...
```{r}
speciesdata <- read.csv("/Users/aleahw/Documents/Master's!!/Fuzzy/FCB_scores_V2.csv")
...
shapiro.test(speciesdata$f) # => p = 0.005867 NORMAL/NOT NORMAL
# Shapiro-Wilk normality test for c
shapiro.test(speciesdata$c) # => p = 0.1595 NORMAL
# Shapiro-Wilk normality test for b
shapiro.test(speciesdata$b) # => p = 0.000394 NOT NORMAL (sig diff from normal dist)
...
res2 <- cor.test(speciesdata$f, speciesdata$b,  method="kendall") #SIG; tau = 0.4496076; p-value = 8.238e-07; z = 4.9297
res2
