...
CALCULATING LOWER, UPPER quartiles for the fuzzy sets (only some traits)
```{r}
#I calculated quartiles for these traits: feed, temp, pO2, Nitrate, phos, growth, size, fecund, salinity, latrange and geogrange; 
...
FCB
#save this: 
NAME<- sprintf("/Users/aleahw/Documents/Master's!!/Fuzzy/FCB_scores_825.2.csv") #saved on Github as "FCB_raw_scores.xlsx"
...
```{r}
speciesdata <- read.csv("/Users/aleahw/Documents/Master's!!/Fuzzy/FCB_scores_825.2.csv") #this file exists as "FCB_raw_scores_xlsx" on Github
...
######################################################
Simplified figures of sensitivity analysis using %
```{r}
#plotting FS sensitivity by individual trait
...
#remove some traits
dat_long <- filter(dat_long, trait_removed %in% c("delta_f.max_size_cm", "delta_f.growth_rate_K", "delta_f.abs_fecundity", "delta_f.macron","delta_f.N_range_µmol.kg", "delta_f.P_range_µmol.kg"))

dat_long$score <- dat_long$score/61*100 #get percentage variation

#graph
plotF <- ggplot(dat_long, aes(x = factor(trait_removed, level = c ("delta_f.max_size_cm", "delta_f.growth_rate_K", "delta_f.abs_fecundity", "delta_f.macron","delta_f.N_range_µmol.kg", "delta_f.P_range_µmol.kg")), y = score)) +
    geom_hline(yintercept=0, linetype="solid", color = "black", size=0.7)+
    geom_violin(fill = "red3")+
      stat_summary(fun.y = median,  geom = "point", color = "black")+
    labs(x = "Trait Removed", y = "F Index Variation (%)")+
...
# setwd("/Users/aleahw/Documents/Master's!!/Fuzzy/Figures")
# ggsave("F_traitsremoved_825.png", plotF, height = 5, width = 8, dpi = 700)
...
#remove some traits
dat_long <- filter(dat_long, trait_removed %in% c("delta_c.feed_eff_trophic","delta_c.N_range_µmol.kg", "delta_c.P_range_µmol.kg", "delta_c.PH_sensitivity_taxa", "delta_c.pO2_min_mbar", "delta_c.sal_range"))

dat_long$score <- dat_long$score/61*100 #get percentage variation

#graph
plotC <- ggplot(dat_long, aes(x = factor(trait_removed, level = c("delta_c.N_range_µmol.kg", "delta_c.P_range_µmol.kg", "delta_c.sal_range", "delta_c.pO2_min_mbar", "delta_c.PH_sensitivity_taxa", "delta_c.feed_eff_trophic")), y = score)) +  
    geom_hline(yintercept=0, linetype="solid", color = "black", size=0.7)+
    geom_violin(fill = "red3")+
      stat_summary(fun.y = median,  geom = "point", color = "black")+
    labs(x = "Trait Removed", y = "C Index Variation (%)")+
   scale_x_discrete(labels = c("5", "6", "7", "8", "9", "10"))+
    theme_classic() +
    theme(axis.text.x = element_text(size = 15, angle = 0, hjust = 1),
          axis.text.y = element_text(size = 15),
          text = element_text(size = 15),
...
#remove some traits
dat_long <- filter(dat_long, trait_removed %in% c("delta_b.growth_rate_K", "delta_b.abs_fecundity","delta_b.lat_range_degrees", "delta_b.geog_range_km2"))

dat_long$score <- dat_long$score/61*100 #get percentage variation

#graph
plotB <- ggplot(dat_long, aes(x = factor(trait_removed, level = c("delta_b.growth_rate_K", "delta_b.abs_fecundity","delta_b.lat_range_degrees", "delta_b.geog_range_km2")), y = score)) +  
    geom_hline(yintercept=0, linetype="solid", color = "black", size=0.7)+
    geom_violin(fill = "red3")+
      stat_summary(fun.y = median,  geom = "point", color = "black")+
    labs(x = "Trait Removed", y = "B Index Variation (%)")+
   scale_x_discrete(labels = c("2", "3", "11", "12"))+
    theme_classic() +
    theme(axis.text.x = element_text(size = 15, angle = 0, hjust = 1),
          axis.text.y = element_text(size = 15),
          text = element_text(size = 15),
...
p <- plot_grid(plotF, plotC, plotB, labels = "AUTO")
# setwd("/Users/aleahw/Documents/Master's!!/Fuzzy/Figures")
ggsave("FCB_facet_sens.png", p, height = 5, width = 8, dpi = 1000)
